...
const TurnBtn = ({ classList, inner, front }) => {
    const btnRef = useRef(null);

    const turnCard = () => {
        inner.current.classList.toggle('rotate-y-180');
        front.current.classList.toggle('hide');
        btnRef.current.classList.toggle('rotate-y-0');
    }
    return (
        <button id='flib-btn' className='slide-flip-btn slide-btn-container' type="button" onClick={() => turnCard()} ref={btnRef}>
